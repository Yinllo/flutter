...
import com.desjf.dsjr.adapter.bankAdapter.BankFundDetailsItemAdapter;
...
public class BankAllFragment extends BaseFragment {
    ...
    private BankFundDetailsItemAdapter myFundDetailsAdapter;
    ...
    private RecyclerViewNoBugLinearLayoutManager linearLayoutManager= new RecyclerViewNoBugLinearLayoutManager(getActivity());
    private int i = 1;//当前页数
    int maxCount=300;//每次加载的记录条数
    ...
    private void getJyjl() {
        showLoadingDialog();
        CallUtil.call(getActivity(), BankHttpUtils.getHttpRequestService().getUserAllMessage(PreferenceCache.getToken(), i + "", maxCount + "", "A"), new CallUtil.MyCallListener() {
            @Override
...
//        linearLayoutManager = new LinearLayoutManager(getActivity());
        if(jyRecordModels==null){
            ...
            if (jyRecordModelList.size() != 0) {
                if (i == 1) {
                    myFundDetailsAdapter = new BankFundDetailsItemAdapter(jyRecordModelList, getActivity());
        ...
        rvAccountFundDetails.setOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE
                        && lastVisibleItem + 1 == myFundDetailsAdapter.getItemCount()&&myFundDetailsAdapter.getItemCount()>=299) {
