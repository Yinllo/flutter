...
/**
 * @author YinL
 * @Describe 网络请求处理类(带失败方法的回调):根据返回的数据进行转换
 */
...
//                if (response.body().toString().contains("nologin")) {// token认证失败
...
//                }
            ...
            public void onFailure(Call<String> call, Throwable t) {
                ...
                myCallListener.onRespnseFailure(call,t);
            }
        });
    }
    public static void Refreshcall(final Context context, Call<String> call,final MyCallListener myCallListener) {
        call.enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
...
//        handler.sendEmptyMessageDelayed(Constant.WHAT_FOOT_GONE, 2000);
    ...
    public abstract static class MyCallListener {
        ...
        public abstract void onRespnseFailure(Call<String> call, Throwable t);
